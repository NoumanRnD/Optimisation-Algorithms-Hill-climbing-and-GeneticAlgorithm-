...
def find_Solution(input_file_name):
    global food_array

    food_array = np.array(read_in_file(input_file_name))
    find_all_slices()
...
def find_all_slices_by_max_cells(cells):
    ...
    for rowStep in range(1, 27):
        ...
        for rowIndex in range(0, food_rows - rowStep + 1):
            for colIndex in range(0, food_cols - colStep + 1):
...
def check_ingridient_constraint(rec=[]):
    ...
    if rec[0] == rec[2]:
        for col in range(rec[1], rec[3] + 1):
            if food_array[rec[0]][col] == 'T':
                count_tomatos += 1
            else:
                count_mushrooms += 1
    elif rec[1] == rec[3]:
        for row in range(rec[0], rec[2] + 1):
            if food_array[row][rec[1]] == 'T':
                count_tomatos += 1
            else:
                count_mushrooms += 1
    else:
...
def read_in_file(file_name):
    ...
    food_rows = int(splitted_header[0])
    food_cols = int(splitted_header[1])
    ...
    max_number_of_slices = int((food_cols * food_rows) / min_cells_per_slice)

    for line in input_file:
        line = line.replace("\n", "")
        food_array.append(list(line))
...
write_output_file("solution_big.txt", find_Solution("big.in"))